...
app.post("/create-account", (req, res) => {
  ...
  user
    ...
    .catch((e) => {
      res
        .status(400)
        .sendFile(path.join(__dirname + "/public/userExists.html"));
    });
});
      ...
      if (!user) {
        ...
        bcrypt.compare(password, user.password, (err, result) => {
          if (result === false) {
            res
              .status(400)
              .sendFile(path.join(__dirname + "/public/invalidPassword.html"));
          } else {
    ...
    User.findOne({ email: req.session.user.email }).then((user) => {
      if (!user) {
        ...
        if (user.ownerInfo[0]) {
          ...
          user.save().then(() => {
            res.sendFile(
              path.join(__dirname + "/public/addressRegistered.html")
            );
          });
        }
      }
  ...
  if (req.session && req.session.user) {
    User.findOne({ email: req.session.user.email }).then((user) => {
      if (!user) {
        ...
        var area = req.body.area;
        User.findOne({
          "ownerInfo.state": state,
          "ownerInfo.district": district,
          "ownerInfo.town": town,
          "ownerInfo.area": area,
        }).then((user) => {
          if (!user) {
        ...
        bcrypt.compare(currentPassword, user.password, (err, result) => {
          if (result === false) {
            res
              .status(400)
              .sendFile(
                path.join(__dirname + "/public/invalidCurrentPassword.html")
              );
          } else {
                ...
                User.updateOne(
                  ...
                  (e, result) => {
                    res
                      ...
                      .sendFile(
                        path.join(
                          __dirname + "/public/passwordChangeSuccess.html"
                        )
                      );
                  }
                );
